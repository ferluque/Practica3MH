...
int main () {
    ...
    string path = "..\\datos";
    ofstream salida;
    salida.open("..\\resultados\\AM-10-0.1.csv");
...
Solution_enteros AM_10_0_1(int M, Problem p,float pls) {
    ...
    while (evaluaciones < 1e5) {
        ...
        if ((generaciones%10==0)&&(generaciones/10>=1)){
            // Se aplica al 10% de los cromosomas
            int num_bl = M*pls;
            vector<int> bls = range(0,M);
            Random::shuffle(bls);
            for (int i=0; i<num_bl ;i++) {
                pair<Solution_enteros,int> par = bl(p,Padres[bls[i]]);
                evaluaciones += par.second;
                Padres[bls[i]] = par.first;
            }
            generaciones /= 10;
        }
        else {
